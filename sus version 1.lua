...
local function useGrace()
    ...
    local graceWorld = workspace.Item2:FindFirstChild("OA's Grace")

    if graceBackpack then
...
local function spawnDeku()
    local prompt = workspace.Map.RuinedCity.Spawn:FindFirstChild("ProximityPrompt")
    if prompt then
...
local function spawnRoland()
    local prompt = workspace.Map.RuinedCity.Spawn:FindFirstChild("ProximityPromptB")
    if prompt then
        fireproximityprompt(prompt)
    end
end
...
local function pickupGloves()
    local gloves = workspace.Item:FindFirstChild("Gloves")
